...
def create_discriminator():
    ...
    x = BatchNormalization()(x)
    x = LSTM(128, return_sequences=True,kernel_regularizer=regularizers.l1(0.89),activation='relu')(x)
#     x = LeakyReLU()(x)
#     x = LSTM(128, return_sequences=True,activation='relu')(x)
    x = LSTM(64, return_sequences=True,activation='relu')(x)
    x = Dropout(0.35)(x)
    x = LSTM(32, return_sequences=True,activation='relu', kernel_regularizer=regularizers.l2(0.019))(x)
