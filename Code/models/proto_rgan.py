...
def create_discriminator():
    ...
    x = LeakyReLU()(x)
    # x = LSTM(40, return_sequences=True,activation='relu')(x)
...
def G_loss(fake_data):
    g_loss = tf.math.reduce_mean(cross_entropy(tf.zeros_like(fake_data), fake_data))
...
def generate_result():
        ...
        batch = np.concatenate([test_sample[start_pos:end_pos, :, :], fill], axis=0)
        ...
        for i in range(1, 20):
            ...
            print('Final Evaluation: tao={:.5}; Accu: {:.4}; Pre: {:.4}; Rec: {:.4}; F1: {:.4}\n'
