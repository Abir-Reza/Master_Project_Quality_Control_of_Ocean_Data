...
def create_generator():
    ...
    x = LSTM(16, return_sequences=True)(x[:, :, :64]) 
    x = LeakyReLU(alpha=0.4)(x)
...
def create_discriminator():
    ...
    x = BatchNormalization()(discriminator_input)
    x = LSTM(64, return_sequences=True)(x[:, :, :64])
    x = LeakyReLU(alpha=0.4)(x)
    x = BatchNormalization()(x)
    x = LSTM(32, return_sequences=True)(x[:, :, :32])
    x = LeakyReLU(alpha=0.4)(x)
    x = BatchNormalization()(x)
...
discriminator.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1,epsilon=1), loss='binary_crossentropy')
generator.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.4,epsilon=1), loss='binary_crossentropy')


def D_loss(real_data, fake_data):
    real_data_loss = tf.math.reduce_mean(cross_entropy(tf.zeros_like(real_data), real_data))
    fake_data_loss = (tf.math.reduce_mean(cross_entropy(tf.ones_like(fake_data), fake_data)))
...
def D_loss_2(real_data, fake_data, augment_label):
    real_data_loss = tf.math.reduce_mean(cross_entropy(tf.zeros_like(real_data), real_data))
    fake_data_loss = (tf.math.reduce_mean(cross_entropy(tf.ones_like(fake_data), fake_data)))
...
def train_step(train_with_augmented = False):
    ...
    for batch_idx in range(total_batch):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            if train_with_augmented:
                disc_loss = D_loss_2(real_input_predictions, generated_input_predictions, batch_label)
            else: 
            ...
            for grad in gradients_of_generator:
                g_grads.append(np.max(grad))
            for grad in gradients_of_discriminator:
                d_grads.append(np.max(grad))

            print('Generator gradient: ',np.mean(g_grads))
            print('Discriminator gradient: ',np.mean(d_grads))
...
def generate_result():
        ...
        tao_max =0.05661212
        for i in range(1, 20):
            tao = float(tao_max - (0.00000001*i))
